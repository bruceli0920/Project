...
// var apiBase = 'http://airtest.rtmap.com/service-api'; // 默认测试接口
...
var pathConfig = {
    server: { // 启动服务相关配置
        ...
        startPath: 'fids/portal.html'
    },
    basepath: __dirname + '/build', // 文件编译 指定根目录
    pageBase: '/fids/', // html中 base标签的 url 便于 引入静态资源 解决 js中跳转 页面的路径问题
    baseDir: '/fids/', // 输出到 根目录下的 文件夹的名称
    output: { // 各个资源 对应的输出路径
        html: '/',
        js: 'js/',
        css: 'css/',
...
function toRlsDir() {
    ...
    if (!!global.wechatConfig) { // 设置 启动 外层服务的 初始化访问页面 gulp tracing --wechat:s --pub test --torls
        global.wechatConfig.startPath = globalConfig.common + 'standard/fids/portal.html';
        console.log('toRlsDir');
    }
}
// 所有任务 执行前的 变量赋值 操作
gulp.task('fids:setValue', function () {
    if (yargs.pub) { // 有 pub 参数
        switch (yargs.pub) {
            case 'rls': // 发布
                pathConfig.pageBase = globalConfig.common + 'standard/fids/';
            ...
            case 'dev': // 开发
                pathConfig.pageBase = globalConfig.common + 'standard/fids/';
                apiBase = 'http://10.10.10.205:8080/service-api';
                console.log('进入开发状态');
                break;
            case 'test': // 测试
                console.log('进入测试状态');
    ...
    if (yargs.ss) { // 启动以wechat/build为根路径的服务
        ...
        //修改根路径 到项目启始页的前缀
        pathConfig.server.startPath = globalConfig.common + 'standard/fids/portal.html';
    }
    //console.log(apiBase);
    console.log('代码打包到 ' + pathConfig.basepath + pathConfig.baseDir + ' 文件夹下');
});
//定义一个testLess任务（自定义任务名称）
gulp.task('fids:css', function () {
...
gulp.task('fids:all', ['fids:css', 'fids:js', 'fids:html', 'fids:images']);

gulp.task('fids:watch', ['fids:all'], function () {
    gulp.watch(__dirname + '/src/sass/*.scss', ['fids:css']);
    gulp.watch(__dirname + '/src/js/*.js', ['fids:js']);
    gulp.watch(__dirname + '/src/*.html', ['fids:html']);
    gulp.watch(__dirname + '/src/images/**/*', ['fids:images']);
});
gulp.task('default', ['fids:setValue', 'fids:css', 'fids:js', 'fids:html', 'fids:images'], () => {
    if (yargs.s || yargs.ss) {
        gulp.start('fids:server');
        console.log('开启服务');
    }
    if (yargs.w) {
        gulp.start('fids:watch');
    }
});
