(function () {
    window.createDetailPage = function (point) {
        if ($(".route_detail").length != 0) {
            ...
            var endName='';
            try{
                endName = Page.RoutePlan.getEnd().name;
            } catch(e) {}
            ...
            routePlanningPage("前往" + " " + endName,function(){
                ...
                $(".route_planning_title .target_time").html("预计:" + parseInt(distanceMeter / 90) + "分"+ parseInt((distanceMeter % 90) * 60 / 90)+"秒");
            });
            ...
            var startName = '';
            try {
                startName = Page.RoutePlan.getStart().name;
            } catch (e) { }

            var $route_start = $("<div class='route_start'>" +
                "<div class='left'><img src='./public/img/info_list_start.png' width='25px' height='25px'/><span>" + startName + "</span></div>" +
                "</div>"); //"<div class='right'></div> + start.attr("floor") + "
            var $route_info = $("<div class='route_info'></div>");

            //var end = $(".set_point_page .set_end");
            //var endName = (end.val() === '我的位置') ? '我的位置' : end.val().split('---')[0];
            //if(end.val() === '我的位置'){
            //    endName = '我的位置';
            //} else {
            //    endName = end.val().split('---')[0] || end.val();
            //}
            var $route_end = $("<div class='route_end'><div class='left'><img src='./public/img/info_list_end.png' width='25px' height='25px'/><span>" + endName + "</span></div>" +
                "</div>");
...
//                            $li.find(".info_list .right .content").html(poiList[i].desc);
                ...
                // 只有楼层没有具体节点的隐藏
                $route_info.find('.list_steps').each(function(_index){
